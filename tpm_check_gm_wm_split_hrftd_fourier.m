...
for first = 1:length(hf(1,1,:));
    for second = 1:length(hf(1,:,1));
        for third = 1:length(hf(:,1,1));
            if hf(first,second,third) > 0;
